...
<main role="main" class="main-content">
    <div class="container-fluid">
        <div class="row">
            <div class="col-12">
                <h2 class="page-title">
                    <a href="<?= $base_url; ?>auditor_data_penugasan" style="text-decoration: none;"><i class="fe fe-arrow-left-circle"></i></a>
                    <?= $page; ?>
                </h2>
            </div>
        </div>
        <div class="row">
            <div class="col-12">
                <form action="auditor_data_penugasan" method="POST">
                    ...
                    <button type="submit" name="editpenugasan" class="btn btn-primary">Simpan</button>
                </form>
            </div>
        </div> <!-- .row -->
    </div> <!-- .container-fluid -->
...
<script>
    $(function() {
        $('#jp').change(function() {
            ...
            if ($('#jp').val() == 'Lainnya') {
                $.get('app/controllers/auditor/default/data_penugasan/dynamic_auditan.php', {
                        jp: $('#jp').val()
                    })
                    .done(function(data) {
    ...
    $(function() {
        $('#auditan').change(function() {
            ...
            if ($('#auditan').val() == 'Instansi Vertikal') {
                $('.input_opd2').remove();
                $.get('app/controllers/auditor/default/data_penugasan/dynamic_auditan2.php', {
                        auditan: $('#auditan').val()
                    })
                    .done(function(data) {
...
<script>
    $(function() {
        $('#auditan').change(function() {
            $('.input_opd').remove();
            if ($('#auditan').val() == 'OPD') {
                $.get('app/controllers/auditor/default/data_penugasan/dynamic_auditan3.php', {
                        auditan: $('#auditan').val()
                    })
                    .done(function(data) {
                        $('div.row_auditan').after(data);
